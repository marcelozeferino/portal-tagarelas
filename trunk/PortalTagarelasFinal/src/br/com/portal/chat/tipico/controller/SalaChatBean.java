...
//					}
					...
					buscaMensagensPais(entry.getValue(), 0);
				}
				conversa.append(mapMensagens.get(entry.getKey()));
	...
	private void buscaMensagensPais(Long idMensagemPaiTemp, Integer nivel){
		String var = "";
		if(mapRelacionamentosInvertido.get(idMensagemPaiTemp) != 0){
			buscaMensagensPais(mapRelacionamentosInvertido.get(idMensagemPaiTemp), nivel + 1);
			if(nivel == 0){
				conversa.append("</div>");
			}
