package com.br.portaltagarelas.interceptor;
	...
	protected void before(ActionInvocation invocation, ValidationAware validationAware) throws Exception {
		...
		if (actionErrors != null && actionErrors.size() > 0) {
			for (String error : actionErrors) {
				if(!validationAware.getActionErrors().contains(error)){
					validationAware.addActionError(error);
				}
			}
		} else validationAware.setActionErrors(null);
		...
		if (actionMessages != null && actionMessages.size() > 0) {
			for (String message : actionMessages) {
				if(!validationAware.getActionMessages().contains(message)){
					validationAware.addActionMessage(message);
				}
			}
		} else validationAware.setActionMessages(null);
